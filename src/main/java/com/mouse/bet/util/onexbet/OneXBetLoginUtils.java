...
/**
 * Utility class for 1xBet login operations
 */
@Slf4j
public class OneXBetLoginUtils {
...
//            passwordField.fill(password);
    ...
    private static Locator findLoginButton(Page page) {
        ...
        try {
            log.debug("Strategy 1: Finding by role and name 'Log in'...");
            loginButton = page.getByRole(AriaRole.BUTTON,
                    new Page.GetByRoleOptions().setName("Log in"));

            if (loginButton.count() > 0 && loginButton.first().isVisible()) {
                log.debug("✅ Login button found via role+name strategy");
        ...
        try {
            log.debug("Strategy 2: Finding by text 'Log in'...");
        ...
        try {
            log.debug("Strategy 3: Finding by class 'auth-dropdown-trigger'...");
            loginButton = page.locator("button.auth-dropdown-trigger");

            if (loginButton.count() > 0 && loginButton.first().isVisible()) {
                log.debug("✅ Login button found via class strategy");
                return loginButton.first();
            }
        } catch (Exception e) {
        ...
        try {
            log.debug("Strategy 4: Checking all buttons for 'Log in' text...");
            ...
            for (int i = 0; i < totalButtons; i++) {
                ...
                if (buttonText != null && buttonText.trim().equals("Log in")) {
                    if (button.isVisible()) {
                        log.debug("✅ Login button found at index {} via all-buttons strategy", i);
                        return button;
                    }
                }
            }
    ...
    private static Locator findSubmitButton(Page page) {
        ...
        try {
            log.debug("Looking for button with type='submit'...");
            submitButton = page.locator("button[type='submit']").first();

            if (submitButton.isVisible(new Locator.IsVisibleOptions().setTimeout(2000))) {
                log.debug("✅ Submit button found (type=submit)");
                return submitButton;
            }
        } catch (Exception e) {
            log.debug("Submit button with type=submit not found");
        }

        // Strategy 2: Buttons inside forms
        try {
            log.debug("Looking for buttons inside forms...");
            ...
            for (int i = 0; i < count; i++) {
                ...
                if (text != null && (text.trim().equals("Log in") || text.trim().equals("LOGIN"))) {
                    if (btn.isVisible()) {
                        log.debug("✅ Submit button found (text='{}') at index {}", text.trim(), i);
                        return btn;
                    }
                }
            }
        } catch (Exception e) {
            log.debug("Could not find submit button in forms: {}", e.getMessage());
        }
